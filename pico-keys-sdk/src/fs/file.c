...
void process_fci(const file_t *pe, int fmd) {
    ...
    else if (pe->type & FILE_TYPE_WORKING) {
        res_APDU[res_APDU_size++] |= pe->file_struct & 0x7;
    }
...
int meta_find(uint16_t fid, uint8_t **out) {
    file_t *ef = search_by_fid(FILE_META, NULL, SPECIFY_EF);
    if (!ef) {
        return CCID_ERR_FILE_NOT_FOUND;
    }
    uint16_t tag = 0x0;
