...
int hsm_key_domain() {
    ...
    if (p1 == 0x0) { //dkek import
        if (apdu.nc > 0) {
            file_t *tf = file_new(FILE_DKEK + p2);
            if (!tf) {
            ...
            file_t *tf = search_dynamic_file(FILE_XKEK + p2);
            if (2 * p2 >= tf_kd_size) {
        ...
        file_t *tf = NULL;
        if ((tf = search_dynamic_file(FILE_DKEK + p2))) {
            if (delete_file(tf) != CCID_OK) {
                return SW_EXEC_ERROR();
            }
        }
        if (p1 == 0x3 && (tf = search_dynamic_file(FILE_XKEK + p2))) {
            if (delete_file(tf) != CCID_OK) {
                return SW_EXEC_ERROR();
            }
        }
        ...
        if (apdu.nc > 0) {
            ...
            free(input);
            int r = pubkeyverify(t54, t54_len, hash, 32, apdu.data, apdu.nc);
            if (r != 0) {
                return SW_CONDITIONS_NOT_SATISFIED();
            }
            file_t *tf = file_new(FILE_XKEK + p2);
            if (!tf) {
                return SW_MEMORY_FAILURE();
            }
